...
export async function POST(request: any, response: any) {
    ...
    try {
        if (session) {
            ...
            console.log("The assigner id is: ", assigner_id)
            await connectMongoDB();
            // const data = await User.findOne({ assigner_id });
            // console.log("Data", data)
            // await Task.create({ assigner_id, assigner_name, task_id, task_name, deadline, assigner_id, points  })
            const tasks = await Task.find({ assigner_id, isCompleted: false, isApproved: false });
        ...
        return NextResponse.json({ message: "Error in assigned task retrival" }, { status: 403 });
    }
