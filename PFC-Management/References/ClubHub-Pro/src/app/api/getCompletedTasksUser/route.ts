...
export async function POST(request: any, response: any) {
    ...
    try {
        if (session) {
            ...
            // await Task.create({ assignee_id, assigner_name, task_id, task_name, deadline, assignee_id, points  })
            const tasks = await Task.find({ assignee_id, isCompleted: true, isApproved: false });
