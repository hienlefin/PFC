  ...
  
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex items-center">
          {/* <span>{row.getValue("assigner_image_url")}</span> */}
         <HoverCardForImage name={row.getValue("assigner_name")} img_url={row.getValue("assigner_image_url")} task_name={row.getValue("task_name")} task_description={row.getValue("task_description")} />
        </div>
      )
    },
    filterFn: (row, id, value) => {
